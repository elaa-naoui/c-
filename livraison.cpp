...
bool Livraison::ajouter() {
    QSqlQuery query;
    query.prepare("INSERT INTO LIVRAISON (ID_LIV, DATE_LIV, ADR_LIV, ETAT_LIV, "
...
bool Livraison::supprimer(int id) {
    QSqlQuery query;
    query.prepare("DELETE FROM LIVRAISON WHERE ID_LIV = :id");
...
QSqlQueryModel* Livraison::afficher() {
    ...
    query.prepare("SELECT ID_LIV, PRENOM_CLIENT, NOM_CLIENT, NUM_CLIENT, ADR_LIV, PRIX_LIV, ETAT_LIV, DATE_LIV FROM LIVRAISON");
    if (query.exec()) {
...
bool Livraison::idExists(int id) {
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM LIVRAISON WHERE ID_LIV = :id");
